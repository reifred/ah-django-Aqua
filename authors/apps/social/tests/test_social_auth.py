...
class SocialAuthenticationTestCase(TestCase):
    ...
    def test_google_login_using_invalid_access_token(self):
        response = self.client.post(
            '/api/social-auth/google/',
            {"user": invalid_google_token}, format='json'
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_social_login_an_existing_social_user_with_valid_data(self):
